...
の **GPLv3 フォーク**です（別リポジトリ）。プレビルドの `MWIFI.BIN`（空スロット版）と
対応ソースは下記から入手できます：

- 本体部品（GPLv3）: https://github.com/tanpang-dev/nds-bootstrap-noslothcv
- **MWIFI.BIN（最新リリース）**: https://github.com/tanpang-dev/nds-bootstrap-noslothcv/releases/latest

ダウンロードした `MWIFI.BIN` を **`device/MWIFI.BIN`** に置くと、「設定」タブの鍵焼きで
あなたの SSID + パスワードを注入できます（鍵は PC 内だけで計算され、保存されません）。
